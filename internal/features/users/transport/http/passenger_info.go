...
func (h *UserHTTPHandler) GetPassengerForDriver(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()
	...
	domainUser, err := h.userService.GetPassengerForDriver(ctx, id)
	if err != nil {
		responseHandler.ErrorResponse(err, "error getting passenger info")
		return
	}
